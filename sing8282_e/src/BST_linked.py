...
class BST:
    ...
    def _are_adjacents_aux(self, parent, v1, v2):
        ...
        if parent is None:
            ...
            if parent._left is None or parent._right is None:
                adjacent = False
            else:
                value1 = parent._left._value
                value2 = parent._right._value
                if v1 == value1 and v2 == value2:
                    adjacent = True
                else:
                    adjacent = self._are_adjacents_aux(
                        parent._right, v1, v2) or self._are_adjacents_aux(parent._left, v1, v2)
    ...
    def furthest(self):
        ...
        if self._root is None:
            ...
            values = [self._root._value, self._root._value]
        else:
            node_l = self._root._left
            node_r = self._root._right

            value_left, value_right = None, None
            while node_l is not None:
                value_left = node_l._value
                node_l = node_l._left

            while node_r is not None:
                value_right = node_r._value
                node_r = node_r._right
