...
class Queue:
    ...
    def __eq__(self, target):
        ...
        """
        result = True
        # assume given two queues are same

        if len(self._values) != len(target):
            # check if two queues have same no. of elements
            result = False
            # False if not

        i = 0
        while result and i < len(self._values):
            # compare each place
    ...
    def combine(self, source1, source2):
        ...
        -------------------------------------------------------
        """
        while len(source1._values) or len(source2._values):
            if len(source1._values) and len(source2._values):
                self.insert(source1.remove())
                self.insert(source2.remove())
            else:
                # if either got empty before the other.
                if len(source1._values):
                    self.insert(source1.remove())
                elif len(source2._values):
                    self.insert(source2.remove())
