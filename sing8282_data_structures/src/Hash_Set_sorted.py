...
class Hash_Set:
    ...
    def insert(self, value):
        ...
        -------------------------------------------------------
        """
        insert = value in self
        if insert is False:
            hash_value = hash(value)
            self._table[hash_value % self._capacity].insert(value)
            self._count += 1
            inserted = True
        else:
            inserted = False
    ...
    def remove(self, key):
        ...
        if contains:
            self._table[hash(key) % self._capacity].remove(key)
            value = key
        else:
            value = None
            print(value)
    ...
    def _rehash(self):
        ...
        -------------------------------------------------------
        """
        new_hs = Hash_Set((self._capacity * 2) + 1)
        for each in self:
            new_hs.insert(each)
        self._table = new_hs._table
        self._capacity = new_hs._capacity
