...
class BST:
    ...
    def _aux_equal(self, node_s, node_t):

        if node_s is None and node_t is None:
            equals = True
        elif node_s._value != node_t._value:
            equals = False
        else:
            equals = self._aux_equal(node_s._right, node_t._right) and self._aux_equal(
                node_s._left, node_t._left)
        return equals
    ...
    def __eq__(self, target):
        ...
        #         equals = False
        #     else:
        #         node_sr = self._root._right
        #         node_tr = target._root._right
        #
        #         while node_sr is not None and equals:
        #             if node_sr._value != node_tr._value:
        #                 equals = False
        #             node_sr = node_sr._right
        #             node_tr = node_tr._right
        #
        #         node_sr = self._root._left
        #         node_tr = target._root._left
        #
        #         while node_sr is not None and equals:
        #             if node_sr._value != node_tr._value:
        #                 equals = False
        #             node_sr = node_sr._left
        #             node_tr = node_tr._left
        #
        # else:
        #     equals = False
        equals = True
        if self._count == target._count:
            equals = self._aux_equal(self._root, target._root)
        else:
            equals = False
    ...
    def parent(self, key):
        ...
        if key in self:
            ...
            if node._value == key:
                value = None
            while node is not None and value is False:
                if node._value == key:
                    value = parent_node._value
                elif node._value > key:
                    parent_node = node
                    node = node._left
                elif node._value < key:
    ...
    def parent_r(self, key):
        ...
        if self._root._value == key:
            ...
            value = self._parent_aux(self._root, None, key)
        return value

    def _parent_aux(self, node, parent_node, key):
        if node._value == key:
            value = parent_node._value
        elif node._value > key:
            value = self._parent_aux(node._left, node, key)
        else:
            value = self._parent_aux(node._right, node, key)
        return value
    ...
    def _leaf_count_aux(self, node):
        ...
        if node is None:
            ...
            count = self._leaf_count_aux(
                node._left) + self._leaf_count_aux(node._left)
    ...
    def two_child_aux(self, node):
        if node is None:
            ...
            if node._left is None and node._right is None:
                ...
                count = 1 + self.two_child_aux(
                    node._left) + self.two_child_aux(node._right)
            else:
                count = self.two_child_aux(
                    node._left) + self.two_child_aux(node._right)
    ...
    def two_child_count(self):
        ...
        count = 0
        count += self.two_child_aux(self._root)
        return count
    ...
    def _one_child_aux(self, node):
        ...
        if node is None:
            ...
            if node._left is None and node._right is None:
                ...
                count = 1 + self._one_child_aux(
                    node._left) + self._one_child_aux(node._right)
            else:
    ...
    def one_child_count(self):
        ...
        count = 0
        count += self._one_child_aux(self._root)
        return count
    ...
    def _is_balanced_aux(self, nodel, noder):
        balanced = True
        if nodel is None and noder is None:
            balanced = True
        elif abs(self._node_height(nodel) - self._node_height(noder)) <= 1:
            if nodel is not None and noder is not None:
                balanced = self._is_balanced_aux(
                    nodel._left, noder._left) and self._is_balanced_aux(nodel._left, noder._left)
        else:
            balanced = False
        return balanced
    ...
    def is_balanced(self):
        ...
        if self._root is not None:
            balanced = self._is_balanced_aux(
                self._root._left, self._root._right)
        return balanced
    ...
    def _is_valid_aux(self, node):
        ...
        if node is None:
            ...
            if node._right is None:
                ...
                right_value = node._right._value

            height = False
            if self._node_height(node) == max(right_height, left_height) + 1:
                height = True

            value = True
            if right_value:
                if right_value > node._value:
                    value = True
                else:
                    value = False
            if left_value:
                if left_value < node._value:
                    value = True
                else:
                    value = False

            if height and value:
                is_valid = self._is_valid_aux(
                    node._right) and self._is_valid_aux(node._left)
            else:
                is_valid = False
    ...
    def is_valid(self):
        ...
        is_valid = self._is_valid_aux(self._root)
    ...
    def _inorder_aux(self, node, values):
        if node is not None:
            self._inorder_aux(node._left, values)
            values.append(deepcopy(node._value))
            self._inorder_aux(node._right, values)
        return
    ...
    def inorder(self):
        ...
        """
        inorder = []
        self._inorder_aux(self._root, inorder)
        return inorder

    def _preorder_aux(self, node, values):
        if node is not None:
            values.append(deepcopy(node._value))
            self._preorder_aux(node._left, values)
            self._preorder_aux(node._right, values)
        return
    ...
    def preorder(self):
        ...
        """
        pre_order = []
        self._preorder_aux(self._root, pre_order)
        return pre_order

    def _post_order_aux(self, node, values):
        if node is not None:
            self._post_order_aux(node._left, values)
            self._post_order_aux(node._right, values)
            values.append(deepcopy(node._value))
        return
    ...
    def postorder(self):
        ...
        """
        post_order = []
        self._post_order_aux(self._root, post_order)
        return post_order
